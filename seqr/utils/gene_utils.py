...
def get_gene(gene_id, user):
    ...
    gene_json = get_json_for_gene(
        gene, user=user, add_dbnsfp=True, add_omim=True, add_constraints=True, add_notes=True, add_mgi=True
    )
    return gene_json
...
def parse_locus_list_items(request_json):
    ...
    gene_ids.update({gene_ids[0] for gene_ids in list(gene_symbols_to_ids.values()) if len(gene_ids)})
